<template>
    <PublicLayout hero-title="Discover Your Next Adventure" url="'/bg/product.jpeg'"
        ...
        <main class="max-w-7xl mx-auto px-4 py-10 text-white grid grid-cols-1 lg:grid-cols-[0.8fr_1fr] gap-16">
            <!-- LEFT: IMAGE GALLERY -->
            <section class="space-y-4">
                ...
                <div class="relative w-[35vw] h-[34vw] border-2 border-primary overflow-hidden rounded-xl cursor-zoom-in"
                    ...
                    <!-- Zoom lens -->
                    <div v-if="isZooming"
                        class="absolute rounded-full border-2 bg-white border-gray-300 w-32 h-auto pointer-events-none z-10"
                        :style="lensStyle"></div>
                </div>

                <!-- Thumbnails -->
                <div
                    class="flex items-center gap-2 mt-2 bg-white/20 backdrop-blur-md border border-white/30 p-1 rounded-xl">
            ...
            <section class="flex flex-col gap-6">
                ...
                <h1 class="text-4xl lg:text-5xl font-bold text-primary">{{ product.name }}</h1>

                <p class="text-gray-400 text-md leading-relaxed">{{ product.description }}</p>
                ...
                <div class="flex flex-col lg:flex-row gap-4 mt-6">
                    <div
                        class="flex items-center justify-between bg-white/20 backdrop-blur-md border border-white/30 rounded-lg px-6 py-4 lg:w-1/3">
